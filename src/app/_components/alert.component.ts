...
export class AlertComponent implements OnInit, OnDestroy {
  ...
  student: FormGroup = this.formBuilder.group({
    ...
    // grade: [null, [Validators.required, Validators.max(12), Validators.min(1)]],
  ...
  get f() {
    return this.organization.controls;
  }
  get g() {
    return this.teacher.controls;
  }
  get h() {
  ...
  ngOnInit() {
    ...
    if (this.data.type == "editClass") {
      this.classGrade = this.data.data.name;
    }
  }
  ...
  createEditClass() {
    if (!this.classGrade || !this.classDivision) {
      return;
    } else {
      this.dialogRef.close({
        grade: this.classGrade,
        division: this.classDivision,
      });
    }
  }

  createEditStudent() {
    if (this.h.name.errors && this.h.email.errors) {
      console.log("False");
			console.log(this.h)
			return;
    }
		console.log(this.h);
		this.dialogRef.close({
			name: this.h.name.value,
			// grade: this.h.grade.value,
			// div: this.h.division.value,
